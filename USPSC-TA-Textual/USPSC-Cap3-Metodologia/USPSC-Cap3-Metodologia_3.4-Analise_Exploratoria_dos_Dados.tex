...
Após a coleta e persistência dos registros no banco de dados PostgreSQL, o corpus foi submetido a uma Análise Exploratória dos Dados (AED) com três objetivos: (i)~caracterizar a dimensão e a composição do conjunto coletado; (ii)~verificar a completude dos campos e identificar valores ausentes; e (iii)~examinar a distribuição do comprimento dos textos e detectar publicações atípicas que pudessem comprometer a etapa de classificação de sentimento. A AED foi implementada em Python como o terceiro estágio do \textit{pipeline} (\texttt{pipeline/03\_eda/}), estruturado em dois módulos Streamlit com responsabilidades complementares, ambos acessados via \texttt{streamlit run pipeline/03\_eda/dashboard.py}:

\begin{itemize}
	\item \texttt{pipeline/03\_eda/eda.py}: responsável pela análise textual do corpus — dimensões do conjunto, completude dos campos, distribuição do comprimento dos textos (caracteres e tokens) e
	identificação de publicações atípicas;
	
	\item \texttt{pipeline/03\_eda/dashboard.py}: responsável pela visualização da distribuição de sentimentos e de relevância financeira entre as publicações já rotuladas, por meio de gráficos de setores interativos.
\end{itemize}

Ambos os módulos obtêm os dados diretamente do banco de dados PostgreSQL via \texttt{shared/database.py}, com o auxílio das bibliotecas \texttt{pandas} \cite{pandas2020}, \texttt{plotly} e \texttt{streamlit}. Os resultados das subseções a seguir são provenientes do módulo \texttt{eda.py}.


\subsection{Dimensões do Corpus e Distribuição por Veículo}
\label{subsec:dimensoes}

O corpus final, exportado após o encerramento da coleta, é composto por \textbf{3.563 publicações}, distribuídas entre dois veículos monitorados conforme o \autoref{qua:dist_veiculo}. O InfoMoney responde pela ampla maioria das publicações (88,5\% do total), enquanto o InvestingBrasil contribui com os 11,5\% restantes. Essa assimetria reflete diferenças estruturais no ritmo de publicação das contas institucionais e deve ser considerada na etapa de construção do índice de sentimento agregado, discutida na \autoref{sec:indice_sentimento}.

\begin{quadro}[htb]
	\centering
	\caption{\label{qua:dist_veiculo}Distribuição do corpus por veículo monitorado}
	\begin{tabular}{|l|r|r|}
		\hline
...
A verificação de valores ausentes foi realizada sobre os campos diretamente coletados via API do X/Twitter. O \autoref{qua:missing} sumariza a completude dos campos analisados, e a \autoref{fig:completude} apresenta a distribuição percentual de valores ausentes de forma visual.

\begin{quadro}[htb]
	\centering
	\caption{\label{qua:missing}Completude dos campos coletados via API}
...
Uma característica estrutural relevante do corpus, evidenciada pela AED, é o comprimento inusualmente elevado dos textos em relação ao formato convencional de publicações do X/Twitter. A mediana de \textbf{778	caracteres} e de \textbf{121 tokens} por publicação — valores muito superiores ao limite padrão de 280 caracteres da plataforma — confirma que a quase totalidade das publicações foi veiculada por meio do campo \texttt{note\_tweet} (texto estendido), disponível para usuários com assinatura X Premium. O \autoref{qua:stats_texto} apresenta as estatísticas descritivas completas.

\begin{quadro}[htb]
	\centering
	\caption{\label{qua:stats_texto}Estatísticas descritivas do comprimento das publicações do corpus}
	\begin{tabular}{|l|r|r|}
		\hline
