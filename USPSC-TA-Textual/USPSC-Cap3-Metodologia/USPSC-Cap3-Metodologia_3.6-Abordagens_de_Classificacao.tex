...
Dois léxicos de polaridade para o português foram considerados para implementação neste trabalho:

\begin{itemize}
	\item \textbf{OpLexicon v3.0} \cite{souza2011oplexicon}: léxico de polaridade para o português brasileiro, construído por meio de um processo semiautomático de expansão léxica com revisão manual. Contém 32.191 entradas com escores de polaridade positiva (+1), negativa (--1) e neutra (0), cobrindo substantivos, adjetivos, verbos, advérbios e expressões emotivas;
	
	\item \textbf{SentiLex-PT} \cite{silva2012sentilex}: léxico de sentimento para o português europeu e brasileiro, com cobertura de 7.014 lemas e 82.347 formas flexionadas, desenvolvido com foco em sentimentos orientados a entidades humanas.
\end{itemize}

O \textbf{OpLexicon v3.0} foi selecionado por duas razões: (i)~abrangência léxica consideravelmente maior (32.191 entradas frente a 7.014 lemas do SentiLex-PT), o que reduz a taxa de \textit{tokens} sem correspondência em textos financeiros; e (ii)~disponibilidade pública irrestrita, garantindo a reprodutibilidade da metodologia proposta.
...
A implementação foi realizada no quinto estágio do \textit{pipeline} por meio da classe \texttt{OpLexiconAnalyzer}, definida em \texttt{pipeline/05\_processing/oplexicon\_analyzer.py}. A classe herda de \texttt{BaseSentimentAnalyzer} — a mesma classe abstrata (ABC) que fundamenta o
\texttt{FinBERTAnalyzer} —, garantindo interface uniforme entre as duas abordagens e extensibilidade para classificadores futuros.
...
O protocolo de \textit{fine-tuning} é implementado no script \texttt{pipeline/05\_processing/bert\_timbau\_fine\_tuner.py} e compreende as seguintes decisões de configuração:

\begin{itemize}
	\item \textbf{Particionamento}: o corpus rotulado é particionado segundo o protocolo descrito na \autoref{subsec:particionamento} --- divisão estratificada 70/15/15 com \texttt{random\_state=42}, persistindo os identificadores de cada conjunto em arquivos auxiliares (\texttt{train\_split.csv}, \texttt{val\_split.csv}, \texttt{test\_split.csv}) na pasta \texttt{models/bert-timbau-sentiment/eval/};
	...
	\item \textbf{Hiperparâmetros}: taxa de aprendizado $2 \times 10^{-5}$, tamanho de lote (\textit{batch size}) 16, comprimento máximo de sequência 128 \textit{tokens}, razão de \textit{warmup} 0,1 e decaimento de pesos (\textit{weight decay}) $10^{-2}$. Esses valores seguem as recomendações dos autores do BERT para \textit{fine-tuning} em conjuntos de dados de tamanho moderado \cite{devlin2019};
	
	\item \textbf{\textit{Early stopping}}: o treinamento é limitado a seis épocas, com critério de parada antecipada (\textit{patience}~=~2) monitorando o F1-Score macro no conjunto de validação. O \textit{checkpoint} com melhor F1-Score macro na validação é selecionado automaticamente como modelo final, prevenindo o \textit{overfitting} à distribuição de treinamento;
...
A inferência é implementada pela classe \texttt{BERTimbauAnalyzer}, definida em \texttt{pipeline/05\_processing/bert\_timbau.py}. Assim como \texttt{FinBERTAnalyzer}, a classe herda de \texttt{BaseSentimentAnalyzer} (\texttt{pipeline/05\_processing/base\_model.py}), garantindo que ambos os analisadores compartilhem o mesmo contrato de interface e possam ser alternados pelo painel de processamento sem modificações adicionais. O protocolo de inferência é análogo ao descrito na \autoref{subsubsec:protocolo_inferencia}, com as seguintes particularidades:
...
\begin{enumerate}
	...
	\item \textbf{Carregamento do modelo}: tokenizador e modelo são carregados diretamente do diretório local via \texttt{AutoTokenizer} e \texttt{AutoModelForSequenceClassification}, encapsulados em objeto \texttt{pipeline} da biblioteca \texttt{transformers} com \texttt{task="text-classification"};
...
A comparação entre as três abordagens --- léxica, FinBERT-PT-BR e BERTimbau ajustado --- será realizada exclusivamente sobre o conjunto de teste \textit{hold-out} definido na \autoref{subsec:particionamento}, que constitui o \textit{gold standard} efetivo da avaliação. Essa restrição é necessária para garantir comparabilidade: como o BERTimbau ajustado viu parte do corpus rotulado durante o treinamento, avaliá-lo sobre o conjunto completo produziria estimativas otimistas de desempenho incomparáveis às das demais abordagens. A implementação das métricas está organizada no sexto estágio do \textit{pipeline} (\texttt{pipeline/06\_evaluation/metrics.py}), invocado via \texttt{make evaluate}. As métricas calculadas são:
