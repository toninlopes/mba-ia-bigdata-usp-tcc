...
O conjunto das etapas descritas nas seções anteriores foi organizado em um \textit{pipeline} integrado de PLN de ponta a ponta (\textit{end-to-end}), implementado em Python. A modularidade da
implementação garante que cada etapa possa ser executada, auditada e substituída de forma independente, o que é um requisito central para a reprodutibilidade da metodologia proposta \cite{santos2023finbert}.
...
\begin{enumerate}
	...
	\item \textbf{Classificação de sentimento}: o corpus pré-processado é submetido ao modelo FinBERT-PT-BR para inferência de polaridade, retornando a classe predita e o vetor de probabilidades para cada publicação, conforme descrito na 	\autoref{sec:abordagens};
	
	\item \textbf{Agregação e análise}: as classificações individuais são agregadas temporalmente para a construção do índice de sentimento, conforme descrito na \autoref{sec:indice_sentimento}.
\end{enumerate}
...
\begin{figure}[htb]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tikzpicture}[
			...
			\node[box, right=of preproc] (classif) {Classificação\\FinBERT};
...
A implementação foi realizada na linguagem \textbf{Python 3.11}, escolhida por sua ampla adoção na comunidade de PLN e aprendizado de máquina e pela disponibilidade de ecossistema maduro de bibliotecas especializadas. O \autoref{qua:bibliotecas} lista as principais dependências utilizadas e suas respectivas funções no \textit{pipeline}.

\begin{quadro}[htb]
	\centering
	\caption{\label{qua:bibliotecas}Principais bibliotecas Python utilizadas no \textit{pipeline} e respectivas funções}
	\begin{tabular}{|p{3.5cm}|p{3cm}|p{7cm}|}
		\hline
		\textbf{Biblioteca} & \textbf{Versão} & \textbf{Função no \textit{Pipeline}} \\
		\hline
		\texttt{requests} & $\geq$ 2.31 &
		Requisições HTTP à API v2 do X/Twitter com Bearer Token \\
		\hline
		\texttt{psycopg2} & $\geq$ 2.9 &
		Interface de comunicação com o banco de dados PostgreSQL \\
		\hline
		\texttt{streamlit} & $\geq$ 1.35 &
		Ferramenta de anotação manual (\textit{Classifier App}) e painel de AED (\texttt{eda.py}) \\
		\hline
		\texttt{pandas} & $\geq$ 2.0 &
		Manipulação tabular do corpus, AED e construção do índice temporal \\
		\hline
		\texttt{transformers} & $\geq$ 4.40 & Carregamento e inferência com o modelo FinBERT-PT-BR \\
		\hline
		\texttt{torch} & $\geq$ 2.2 &
		\textit{Backend} de inferência neural (PyTorch) \\
		\hline
...
O código-fonte do \textit{pipeline} está organizado como um pacote Python versionado, com dependências declaradas em arquivo \texttt{requirements.txt}, de modo a facilitar a reprodução dos
experimentos em ambientes distintos.